...
import training as engine # Using training.py as the engine
from models import FASCANet

if __name__ == "__main__":
    print("\n" + "="*70)
    print("FASCANet BENCHMARKING & ABLATION")
    print("="*70)
    cleanup()
    ...
    if all_results_buffer:
        ...
        grouped = df.groupby(['noise_type', 'method'])[['psnr', 'ssim', 'fsim', 'vif', 'ms_ssim', 'fom']].agg(['mean', 'std'])
        print(grouped)
        grouped.to_csv(os.path.join(Config.OUTPUT_DIR, "final_fascanet_summary.csv"))
